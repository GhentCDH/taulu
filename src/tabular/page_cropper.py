...
class PageCropper:
    def __init__(self, target_hue = 12, target_s = 26, target_v = 230, tolerance = 40, margin = 140, split = 0.5, split_margin = 0.06):
        """
    ...
    def _create_hue_mask(self, image: MatLike):
        ...
        lower_hue = np.array([max(0, self._target_hue - self._tolerance), max(0, self._target_s - self._tolerance), max(0, self._target_v - self._tolerance)])
        upper_hue = np.array([min(180, self._target_hue + self._tolerance), min(255, self._target_s + self._tolerance), min(255, self._target_v + self._tolerance)])
