...
impl TableGrower {
    ...
    fn approximate_best_step(&self, current: Coord, step: Step) -> Option<Point> {
        let gaussian_weights = gaussian_1d(self.look_distance * 2, None);
    ...
    fn similar_neighbouring_steps(&self, current: Coord, step: Step) -> Option<Vec<Point>> {
        let header_step = &self.header_based_step_from_coord(current, step)? - &self[current]?;
        let mut steps = vec![header_step; self.look_distance * 2];
        ...
        neighbours_one
            ...
            .for_each(|(i, nb)| {
                ...
                {
                    steps[i] = &next_point - &current_point;
                }
            });
